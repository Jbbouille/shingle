...
import org.shingle.extensions.JavaxValidationExtension;
import org.shingle.extensions.ShingleExtension;
import org.shingle.extensions.SwaggerExtension;
import org.shingle.extensions.XmlJavaTypeAdapterExtension;
import org.shingle.model.Documentation;

@Mojo(name = "generate-shingle-doc", requiresDependencyResolution = ResolutionScope.COMPILE_PLUS_RUNTIME)
public class TheMojo extends AbstractMojo {
    ...
    private void doExecute() throws Exception {
        ...
        generateShingleDoc(classes.stream().collect(toSet()), classLoader);
    }
    ...
    private void generateShingleDoc(Set<Class<?>> classes, URLClassLoader classLoader) {
        Shingle shingle = Shingle.builder()
                                 .extensions(asList(new JavaxValidationExtension(),
                                                    ...
                                                    new ShingleExtension()))
                                 .extensions(extensions(classLoader))
                                 .basePath(apiBasePath)
                                 .build();

        Documentation documentation = new DocumentationBuilder(shingle, majorVersion()).execute(classes);
    ...
    private String majorVersion() {
        String version = project.getPlugin("org.shingle:shingle-maven-plugin").getVersion();
